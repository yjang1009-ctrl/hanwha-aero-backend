...
async def refresh_price(kis: KISClient):
    while True:
        try:
            ...
            print("price refresh error:", repr(e))
            traceback.print_exc()
...
async def refresh_chart_and_flow(kis: KISClient):
    while True:
        # 차트와 수급을 서로 독립적으로 시도 - 한쪽이 실패해도 다른 쪽은 계속 갱신되게 함
        for p in ["D", "W", "M", "Y"]:
            try:
                cache["chart"][p] = await kis.get_period_chart(STOCK_CODE, period=p)
            except Exception as e:
                print(f"chart[{p}] refresh error:", repr(e))
                traceback.print_exc()
        ...
        try:
            ...
            print("investor_trend refresh error:", repr(e))
            traceback.print_exc()
...
async def refresh_news():
    while True:
        try:
            ...
            print("news refresh error:", repr(e))
            traceback.print_exc()
...
class UTF8JSONResponse(JSONResponse):
    """한글이 브라우저에서 깨지지 않도록 charset을 명시합니다."""
    media_type = "application/json; charset=utf-8"


app = FastAPI(title="한화에어로 워치 백엔드", lifespan=lifespan, default_response_class=UTF8JSONResponse)
...
@app.get("/")
def dashboard():
    """대시보드 페이지. 브라우저에서 서버 주소 그대로 접속하면 바로 보입니다."""
