...
class KISClient:
    ...
    async def _get(self, path: str, tr_id: str, params: dict) -> dict:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code != 200:
                # 실패 원인을 그대로 노출 - KIS는 200이어도 바디 안에 rt_cd 에러코드를 담는 경우도 있어 같이 확인
                raise RuntimeError(f"KIS API {path} 실패 status={resp.status_code} body={resp.text[:500]}")
            data = resp.json()
            if data.get("rt_cd") not in (None, "0"):
                raise RuntimeError(f"KIS API {path} rt_cd={data.get('rt_cd')} msg={data.get('msg1')} body={str(data)[:500]}")
            return data
    ...
    async def get_period_chart(self, code: str, period: str = "D", count_days: int = 100) -> list[dict]:
        ...
        # 너무 먼 과거로 잡으면 KIS 서버가 500을 내는 경우가 있어 기간을 넉넉히만 잡음(약 5년)
